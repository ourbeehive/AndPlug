...
public class OkHttpClientManager {
    ...
    private void deliveryResult(ResultCallback callback, Request request) {
        ...
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(final Response response) {
                try {
                    ResponseJson json = JSONObject.parseObject(response.body().bytes(), ResponseJson.class);
                    if (resCallBack.mType == String.class) {
                        sendSuccessResultCallback(json.getData(), resCallBack);
                    } else if (resCallBack.mType == ResponseJson.class) {
                        sendSuccessResultCallback(json, resCallBack);
                    } else {
                        Object o = JSON.parseObject(json.getData(), resCallBack.mType);
                        sendSuccessResultCallback(o, json.getPages(), resCallBack);
                    }
    ...
    private void sendSuccessResultCallback(final Object object, final ResponsePage page, final ResultCallback callback) {
        mDelivery.post(new Runnable() {
            @Override
            public void run() {
                callback.onResponse(object, page);
                callback.onAfter();
            }
        });
    }
    ...
    public static abstract class ResultCallback<T> {
        ...
        public abstract void onResponse(T response, ResponsePage page);
    }
    ...
    private final ResultCallback<String> DEFAULT_RESULT_CALLBACK = new ResultCallback<String>() {
        ...
        @Override
        public void onResponse(String response, ResponsePage page) {
